...
class CoDec(EC.CoDec):
    ...
    def __init__(self, args, min_index_val=-128, max_index_val=127):  # ???
        ...
        if self.encoding:
            ...
            with open(f"{args.input}_QSS.txt", 'r') as f:
                self.QSS = int(f.read())
                logging.info(
                    f"Read QSS={self.QSS} from {self.args.output}_QSS.txt")
        self.Q = Quantizer(
            Q_step=self.QSS, min_val=min_index_val, max_val=max_index_val)
        # We suppose that the representation of QSS requires 1 byte.
        self.output_bytes = 1
    ...
    def encode(self):
        ...
        logging.debug(
            f"k.shape={k.shape} k.dtype={k.dtype} k.max={np.max(k)} k.min={np.min(k)}")
        self.write(k)
        # self.save(img)
